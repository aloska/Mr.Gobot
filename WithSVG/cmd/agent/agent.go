...
func (a* Agent) checkpaths() bool{
	...
	for _, file := range files {
		if file.IsDir(){
			...
			if match, _ := regexp.MatchString("(Neuron-[0-9]+.neurons|Synapse-[0-9]+.chemical|GenNeuron-[0-9]+.genes)",
					file.Name()); !match{
						serr:="В корне /Senses могут быть файлы формата Neuron-XXX.neurons GenNeuron-XXX.genes Synapse-XXX.chemical"
	...
	for _, inp:=range inputs{
		...
		for _, file := range files {
			if file.IsDir(){
				...
				pat:="(Data.data|GenData.genes|GenReceptor-[0-9]+.genes|Receptor-[0-9]+.receptors|Neuron-[0-9]+.neurons|Synapse-[0-9]+.chemical|GenNeuron-[0-9]+.genes)"
	...
	for _, file := range files {
		if file.IsDir(){
			...
			//должны быть только файлы  Neuron-XXX.neurons GenNeuron-XXX.genes Synapse-XXX.chemical
			serr:="В корне /Actions не должно быть никаких файлов"
			pterm.Warning.Println("Что это за файл? "+file.Name())
			pterm.Error.Println(serr)
			a.log.Error(serr,zap.String("/Actions", "Не должно быть файла "+file.Name()))
			return false
		}
	}
	...
	for _, inp:=range inputs{
		...
		for _, file := range files {
			if file.IsDir(){
				...
				//должны быть только файлы  Neuron-XXX.neurons GenNeuron-XXX.genes Synapse-XXX.chemical
				pat:="(DataOut.data|GenDataOut.genes|GenPreffector-[0-9]+.genes|Preffector-[0-9]+.preffectors|Neuron-[0-9]+.neurons|Synapse-[0-9]+.chemical|GenNeuron-[0-9]+.genes)"
				if match, _ := regexp.MatchString(pat, file.Name()); !match{
		...
		if gen1 && gen2{
			...
			serr:="В /Actions/Effector-xxx/ должен быть GenPreffector-0.genes и GenDataOut.genes обязательно"
			pterm.Error.Println(serr)
	...
	for _, inp:=range inputs{
		...
		for _, file := range files {
			if file.IsDir(){
				...
				//должны быть только файлы  Neuron-XXX.neurons GenNeuron-XXX.genes Synapse-XXX.chemical
				pat:="(Neuron-[0-9]+.neurons|Synapse-[0-9]+.chemical|GenNeuron-[0-9]+.genes)"
				if match, _ := regexp.MatchString(pat, file.Name()); !match{
		...
		if gen1{
			...
			serr:="В /Brain/Core-xxx/ должен быть GenNeuron-0.genes "
			pterm.Error.Println(serr)
