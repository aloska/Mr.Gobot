...
type DataInput struct {
	...
	/*
	ЭТО ВСЕ НИЖЕИЗЛОЖЕННОЕ ХОРОШО, КОНЕЧНО, БУМАГА НАПИСАНА ВЕРНО, НО НАРУШИТЕЛЬ - ОН НЕ НАРУШИТЕЛЬ, А РАБОТНИК ВЫСОКОЙ КУЛЬТУРЫ БЫТА))
	ПОТОМУ ЧТО:
	Можно, конечно сделать какие угодно рецепторы, но вот за рецепторами стоят НЕЙРОНЫ, а нейроны срабатывают либо нет.
	Это значит, что возможна ситуация, когда число 34 не даст никакой реакции, а 35 даст большую реакцию.
	Поэтому такие типы данных и рецепторы специальных типов надо использовать с осторожностью!!!
	Внимание! Лучше всего использовать рецепторы универсальные и универсальный тип данных, ПОБИТОВЫЕ
	тогда каждый бит будет значимым и будет выдавать какой-то результат.
	Например, для байта, где существенно старшинство битов можно на старшие биты вешать больше рецепторов, а  на младшие меньше
	Но даже тогда по младшему биту пройдут вещества в нейроны.
	А для строк, типа руны, биты все равнозначны

	И еще одно внимание! Можно-таки типы данных использовать специальные, но тогда реализация рецепторов желательна такой, чтобы
	они в этих специальных типах реагировали на отдельные биты. Например, т.к. у рецепторов по 32 аксона, можно сделать так,
	чтобы каждый аксон отвечал за свой бит.
